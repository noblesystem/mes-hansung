<%@ Page Title="" Language="C#" MasterPageFile="~/Mtpage.Master" AutoEventWireup="true" CodeBehind="page6-1.aspx.cs" Inherits="hansung.page6.page6_1" %>
...
<!-- 발주, 통관/입고 버튼들 ==================================================================================== -->
                            <ul class="ul-button" style="">
                                ...
                                <li>
                                    <div class="div-btn-header" onclick="fn_row_add()"><span class="span-title">행추가(대체)</span></div>
                                </li>
                                <li>
                                    <div class="div-btn-header" onclick="fn_row_del()"><span class="span-title">행삭제(대체)</span></div>
                                </li>
                            </ul>
...
<!-- 대체품목 ==================================================================================== -->
    ...
    <script>
        ...
        $(document).ready(function () {
            ...
            $('#gucd').zentSetCombo('C020', '', '==선택=='); $('#gucd').val('B');
            $('#iccd').zentSetCombo('C021', '', '==선택=='); $('#iccd').val('C');
            ...
            jQuery("#grid2").jqGrid({
                colNames: ['mode', 'stno', '번호', '품목코드', '품목명', '규격', '매수', 'iqty(B)', '출고수량(B)',
                    '시리얼 No', 'statuscd', '불량유형', '첨부파일', '항목비고', 'useyn'],
                colModel: [
            ...
            jQuery("#grid3").jqGrid({
                ...
                onSelectRow: function (rowid, status, e) {
                    ...
                    //$this.jqGrid('setGridParam', { cellEdit: false });
                    //if (rowid && rowid !== lastsel) {
                    //    jQuery('#grid3').jqGrid('restoreRow', lastsel);
                    //    jQuery('#grid3').jqGrid('editRow', rowid, true);
                    //    lastsel = rowid;
                    //}
                },
                //loadonce: true,
                ...
                afterSaveCell: function (rowid, cellname, value, iRow, iCol) {
                    if (cellname == "totamt") {
                        ...
                        var supplyamt = parseInt(totamt / 1.1);
                        var vat = totamt - supplyamt;
                        $("#grid3").jqGrid('setRowData', rowid, { 'supplyamt': supplyamt });
                        $("#grid3").jqGrid('setRowData', rowid, { 'vat': vat });
                    }
                },
                afterEditCell: function (rowid, cellname, value, iRow, iCol) {
                    //if (cellname == 'tongdt') { $("#" + iRow + "_tongdt", "#grid3").datepicker({ dateFormat: "yy.mm.dd" }); }
                    //if (cellname == 'passdt') { $("#" + iRow + "_passdt", "#grid3").datepicker({ dateFormat: "yy.mm.dd" }); }
                    //if (cellname == 'ftdt') { $("#" + iRow + "_ftdt", "#grid3").datepicker({ dateFormat: "yy.mm.dd" }); }
                    //if (cellname == 'conbandt') { $("#" + iRow + "_conbandt", "#grid3").datepicker({ dateFormat: "yy.mm.dd" }); }
                    //if (cellname == 'inyodt') { $("#" + iRow + "_inyodt", "#grid3").datepicker({ dateFormat: "yy.mm.dd" }); }
                    //if (cellname == 'indt') { $("#" + iRow + "_indt", "#grid3").datepicker({ dateFormat: "yy.mm.dd" }); }
                    //if (cellname == 'proddt') { $("#" + iRow + "_proddt", "#grid3").datepicker({ dateFormat: "yy.mm.dd" }); }
                    //$("#" + rowid + "_" + cellname).blur(function () {
                    //    $("#tong_grid").jqGrid("saveCell", iRow, iCol);
                    //});
                },
                ...
                */
                ondblClickRow: function (rowid, row, col) {
                    var cm = jQuery("#grid3").jqGrid("getGridParam", "colModel");
                    if (cm[col].name == "itemnm") {
                        var searchword = $('#grid3').jqGrid('getCell', rowid, 'itemnm');
                        var cstno = $('#cstno').val();
                        pop_item('', 'grid3', rowid);
                    }
                },
                //rowNum: 10,
                //rowList: [10, 20, 30],
            ...
            jQuery("#grid3").jqGrid('navGrid', '#grid3_pager', { edit: false, add: false, del: false });
